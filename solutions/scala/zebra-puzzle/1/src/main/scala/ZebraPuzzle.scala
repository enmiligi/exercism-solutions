object ZebraPuzzle {
  ...
  lazy val solutions = for {
    colorPerm <- colors.permutations.filter(colorRules)
    nationPerm <- nations.permutations.filter(nationRules(colorPerm, _))
    petPerm <- pets.permutations.filter(petRules(colorPerm, nationPerm, _))
    beveragePerm <- beverages.permutations.filter(
      beverageRules(colorPerm, nationPerm, petPerm, _)
    )
    cigarettePerm <- cigarettes.permutations.filter(
      cigaretteRules(colorPerm, nationPerm, petPerm, beveragePerm, _)
    )
  } yield Solution(
