...
service /travel on new http:Listener(9090) {
    ...
    resource function post arrangeTour(@http:Payload TourArrangement arrangement)
        returns Result|ErrorResult|error {
        // Extract Travel infomation from the travel reservation request
        string name = arrangement.name;
        string arrivalDate = arrangement.arrivalDate;
        string departureDate = arrangement.departureDate;
        ...
        ServiceResponse airlineResponse = check airlineReservationEP->post("/reserve", reservation);
        ...
        if airlineResponse.status == FAILED {
            return {body: {message: "Failed to reserve airline! Provide a valid 'preference' for 'airline' and try again"}};
        }

        // Follow the same steps for 'Hotel' and 'Car Rental' services.
        // Send a POST request to the /reserve endpoint of the `Hotel` service.
        reservation.preference = arrangement.preference.accomodation;
        ServiceResponse hotelResponse = check hotelReservationEP->post("/reserve", reservation);
        if hotelResponse.status == FAILED {
            return {body: {"message": "Failed to reserve hotel! Provide a valid 'preference' for 'accommodation' and try again"}};
        }
        // Send a POST request to the /rent endpoint of the `Car Rental` service.
        reservation.preference = arrangement.preference.car;
        ServiceResponse carResponse = check carRentalEP->post("/rent", reservation);
        if carResponse.status == FAILED {
            return {body: {"message": "Failed to rent car! Provide a valid 'preference' for 'car' and try again"}};
        }
        ...
        return {"message": "Congratulations! Your journey is ready!!"};
    }
}
